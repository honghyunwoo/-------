...
class CompositionConfig:
    ...
    # 자막 (한글 지원 폰트 파일 경로 또는 시스템 폰트명)
    subtitle_font: str = "/usr/share/fonts/truetype/wqy/wqy-zenhei.ttc"
...
class VideoComposer:
    ...
    def _concatenate_broll(
        ...
        # 길이가 부족하면 마지막 클립 반복
        while total_duration < target_duration and video_clips:
            remaining = target_duration - total_duration
            last_clip = video_clips[-1]

            if remaining > 0:
                loop_clip = last_clip
                if loop_clip.duration > remaining:
                    loop_clip = loop_clip.subclipped(0, remaining)
    ...
    def _create_text_clip(
        ...
        # MoviePy 2.x: fontsize → font_size, align → text_align
        txt_clip = TextClip(
            text=text,
            ...
            method='caption',
            size=(self.config.width - 100, None),  # 좌우 여백
            text_align='center'
        )

        # 위치 설정
        x_pos = self.config.subtitle_position[0]
        y_ratio = self.config.subtitle_position[1]
        y_pos = int(self.config.height * y_ratio)

        # MoviePy 2.x: set_* → with_*
